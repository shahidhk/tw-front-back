...
# These functions will need to be convert to tasks instead of views
...
def update_asset_role(request):
    ...
    for entry in cloned_assets: # populate dict, keys = role number
        if entry.role_number in parent_mtnoi.keys():
            result_str = result_str + entry.role_number + ' has duplicates in the cloned table\n'
        else:
    ...
    with transaction.atomic():
        for role in base_roles:
            try:
                ...
                print(parent_mtnoi[role.updatable_role_number].parent_role_number)
...
def test(request):
    ...
    with transaction.atomic():
        for asset in base_assets:
            asset.asset_serial_number = 'role # ' + str(asset.initial_project_asset_role_id_id) + 'asset serial number'
            asset.save()
